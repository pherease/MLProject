...
SAVE_PATH = "model_load_this.keras"
...
dyn_cb = utils.DynamicMinDelta(reduce_cb = lr_cb, ratio=0.025)
...
if os.path.exists(SAVE_PATH):
    # ❶ Load full model (+ optimizer, compile config, metrics)
    model = tf.keras.models.load_model(
        SAVE_PATH,
        custom_objects = custom_objects,
        compile        = True        # restores optimizer state!
    )
    print(f"🔄  Model, weights, and optimizer state loaded from {SAVE_PATH}")
else:
...
prev_epochs     = model.optimizer.iterations.numpy() // steps_per_epoch
print(f"Previous run completed {prev_epochs} epochs")

history = model.fit(
    train_ds,
    validation_data = val_ds,
    epochs          = 100,   # train 100 *more* epochs
    initial_epoch   = prev_epochs,         # ← key line for resuming
    callbacks = [
        early_dual_cb,
        lr_printer,
        csv_logger_cb(),
